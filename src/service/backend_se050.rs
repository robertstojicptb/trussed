...
impl ServiceBackend for Se050Wrapper {
	...
	// generate p256 key with &[0x03]) -  NIST P-256 
			...
			let mut bytes = Message::new();
			bytes.resize_default(request.count).unwrap();				
		   self.device.write_ec_key( &[0x20, 0xe8, 0xa1, 0x01], &[0x03], self.delay,); 
		   Ok(Reply::RandomBytes(reply::RandomBytes { bytes } ))
	   }
	...
	// generate ed255 key with &[0x40]) -  //ED255
		...
		let mut bytes = Message::new();
		bytes.resize_default(request.count).unwrap();				
	   self.device.write_ec_key(&[0x20, 0xe8, 0xa1, 0x02], &[0x40], self.delay,); 
	   Ok(Reply::RandomBytes(reply::RandomBytes { bytes } ))
   }
...
//TEST SEO50 primitive    read_secure_object   &[0x20, 0xe8, 0xa1, 0x02]
	...
	// bytes.resize_default(request.count); //SLICE LENGTH 32 da 4byte		
	 bytes.resize_default(32); //SLICE LENGTH 32 da 4byte
	//bytes.resize_default(request.count).unwrap();
...
//TEST SEO50 primitive    read_secure_object   &[0x20, 0xe8, 0xa1, 0x01]
	// generate p256 key with &[0x03]) -  //P256

	else if request.count == 65 {

		let mut bytes = Message::new();
		// bytes.resize_default(request.count); //SLICE LENGTH 32 da 4byte		
		  bytes.resize_default(2); //SLICE LENGTH 32 da 4byte
		 // bytes.resize_default(request.count).unwrap();
	// bytes.resize_default(request.count).unwrap();
